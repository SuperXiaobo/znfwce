<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      ...
      <el-form-item label="注册电话号码" prop="phonenum">
        <el-input
    ...
    <el-table v-loading="loading" :data="faceList" @selection-change="handleSelectionChange">
      ...
      <el-table-column label="注册电话号码" align="center" prop="phonenum" />
      <el-table-column label="注册时间" align="center" prop="registertime" width="180">
        <template slot-scope="scope">
          <span>{{ parseTime(scope.row.registertime, '{y}-{m}-{d}') }}</span>
        </template>
      </el-table-column>
      <el-table-column label="人脸数据" align="center" prop="faceid" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        ...
        <el-form-item label="注册电话号码" prop="phonenum">
          <el-input v-model="form.phonenum" placeholder="请输入注册电话号码" />
        </el-form-item>
        <el-form-item label="注册时间" prop="registertime">
          <el-date-picker clearable size="small"
...
<script>
import { listFace, getFace, delFace, addFace, updateFace, exportFace } from "@/api/system/face";
...
export default {
  ...
  data() {
    return {
      ...
      rules: {
        ...
        phonenum: [
          { required: true, message: "注册电话号码不能为空", trigger: "blur" }
        ],
        registertime: [
          { required: true, message: "注册时间不能为空", trigger: "blur" }
        ],
        faceid: [
          { required: true, message: "人脸数据不能为空", trigger: "blur" }
        ]
      }
    };
  },
  ...
  methods: {
    ...
    handleDelete(row) {
      ...
      this.$confirm('是否确认删除【请填写功能名称】编号为"' + names + '"的数据项?', "警告", {
          confirmButtonText: "确定",
          cancelButtonText: "取消",
          type: "warning"
        }).then(function() {
          return delFace(names);
        }).then(() => {
          this.getList();
